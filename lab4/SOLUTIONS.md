...
oc create secret docker-registry nexus-docker-auth --docker-server=nexus-docker.ocp.hpb.tn.hr --docker-username=admin --docker-password=<nexus_password>
...
spec:
  ...
  tasks:
    - name: git-clone
      params:
        - name: url
          value: 'https://github.com/true-north-engineering/hpb-educ-src.git'
        - name: revision
    ...
    - name: s2i-java
      params:
        ...
        - name: TLSVERIFY
          value: 'true'
        - name: IMAGE
          value: 'nexus-docker.ocp.hpb.tn.hr/todo-api:user1'
        - name: BUILDER_IMAGE
